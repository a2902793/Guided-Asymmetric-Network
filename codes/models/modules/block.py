...
class Dense_MB(nn.Module):
    ...
    def __init__(self):
        ...
        # Stem
        in_channels = 64  # rgb
        out_channels = round_filters(40, self._global_params)  # number of output channels
        # self._conv_stem = Conv2d(in_channels, out_channels, kernel_size=3, stride=1, bias=True)
        ...
        out_channels = round_filters(40, self._global_params)
        Conv2d = get_same_padding_conv2d(image_size=image_size)
    ...
    def forward(self, x):
        ...
        """
        # Convolution layers
        x_0 = self.extract_features(x, 0)
        x_1 = self.extract_features(torch.cat((x, x_0), 1), 1)
        x_2 = self.extract_features(torch.cat((x, x_0, x_1), 1), 2)
        x_3 = self.extract_features(torch.cat((x, x_0, x_1, x_2), 1), 3)
